...
Before we can do that however, we need a few key R packages installed, namely: `shiny` and (for convenience) `htmltools`. `shiny` is, perhaps unsurprisingly, the core workhorse package of most Shiny apps. One huge benefit of Shiny is that you can use any R package to runs R code, which makes things like manipulating data or plotting familiar. But the only required package to create a shiny app is `shiny`.
